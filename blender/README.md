...
> 文件查看器通常只展示 `.blend` 的静态内容；实际播放请在 Blender 时间线中进行。动画将每帧角位移限制在可见叶片节距的一半以下（燃气发生器 5°/帧，自由动力转子 4°/帧；最小转动排半节距 5.45°，为 HPC 第 16 级 33 片），避免重复叶排造成“像没动/倒转”的视觉频闪。它仅表示两个转子总成的视觉运动，故意使用低得多的显示速度，**不是**实际 LM2500 RPM、动力学、温度、振动、控制或性能仿真。
...
本场景以 Blender 4.5.3 LTS 的 `bpy` runtime 生成、重新打开并通过结构核验（18 项全过，JSON 见 `../evidence/blender_validation.json`；`.blend` 文件头 `v405` 对应 4.5 系列）。核验器检查的是场景对象层级和已公开的级数/喷嘴数量声明，不是工程性能验证：
...
通过时，核验器将确认：**16** 个压气机转子级、**6** 个 VSV 静子级、**30** 个喷嘴体、**2** 个 HPT 转子级、**6** 个自由动力涡轮转子级，以及嵌入的范围声明。它也检查本次细节通道中的根部平台、VSV 同步导轨、喷嘴雾化杯、轴承保持架、检修盖板、细节相机和两个旋转控制器。生成器对各叶排的保守轴向/周向包络实施正间隙守卫；核验输出会列出三个转子区的最小可视间隙。该检查避免网格穿插，**不是**气动或结构验证。命令也会输出 Blender 版本、对象/网格总数和场景 metadata，便于审计。除整环燃油总管（30 路喷嘴均从其引出，为表达分配关系有意保留整环，见生成器注释）外，切除扇区内无悬浮几何：装在剖切机匣上的环件均为 240° 弧段，机匣孔阵跳过切除扇区，点火器、VSV 拉杆与引气管起止点均落在保留扇区内。

## 重新生成

生成器不依赖外部插件或下载素材，在 Blender 3.6 LTS+ 中可运行；交付的 `.blend` 用 Blender 4.5.3 LTS 验证（`evidence/blender_validation.json`）。默认构建保留完整可见叶排密度和 30 个喷嘴：
...
| `--samples N` | 覆盖 Cycles 采样数（0=按 quick/完整预设取 64/192）。 |
| `--resolution-scale 25..200` | 按百分比调整 1920×1080 输出。随附的两张预览图即 1920×1080 默认全分辨率（192 采样＋降噪，见本表 `--samples` 行）。 |
