...
> 文件查看器通常只展示 `.blend` 的静态内容；实际播放请在 Blender 时间线中进行。动画将每帧角位移限制在可见叶片节距的一半以下，避免重复叶排造成“像没动/倒转”的视觉频闪。它仅表示两个转子总成的视觉运动，故意使用低得多的显示速度，**不是**实际 LM2500 RPM、动力学、温度、振动、控制或性能仿真。
...
通过时，核验器将确认：**16** 个压气机转子级、**6** 个 VSV 静子级、**30** 个喷嘴体、**2** 个 HPT 转子级、**6** 个自由动力涡轮转子级，以及嵌入的范围声明。它也检查本次细节通道中的根部平台、VSV 同步导轨、喷嘴雾化杯、轴承保持架、检修盖板、细节相机和两个旋转控制器。生成器对各叶排的保守轴向/周向包络实施正间隙守卫；核验输出会列出三个转子区的最小可视间隙。该检查避免网格穿插，**不是**气动或结构验证。命令也会输出 Blender 版本、对象/网格总数和场景 metadata，便于审计。
