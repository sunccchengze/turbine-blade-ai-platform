...
| `AGENT宪法级准则.md` | 降低 LLM 编码常见错误的行为准则（原名「最高优先级AGENT必须遵守的宪法级文件 - 副本.md」） | 疑似第三方，见下 |
...
### 关于 `AGENT宪法级准则.md` 的存疑

正文是英文的通用 LLM 编码准则（Think Before Coding / Simplicity First 等），
原文件名带「- 副本」，**很可能抄自某个上游而非本人原创**。
暂存此处，**待本人确认来源**：若属第三方，应移入 `NOTICES.md` 登记并注明出处。
