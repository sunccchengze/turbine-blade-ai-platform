...
用户本机最终 cfg 审计确认：`TURBOMACHINERY_KIND=AXIAL`、`TURBO_PERF_KIND=(COMPRESSOR)`、`MARKER_TURBOMACHINERY=(INLET,OUTLET)`、`PERFORMANCE_AVERAGE_PROCESS_KIND=MASSFLUX`、`MIXEDOUT_COEFF`、`MARKER_GILES=(OUTLET,RADIAL_EQUILIBRIUM,...)` 均存在，且 marker 与网格一致。工作 cfg 生成器现补入 SU2 官方 turbomachinery 性能分析需要的 `MARKER_ANALYZE=(INLET,OUTLET)`；原始课程 cfg 不修改。性能配置 Gate 将在 clean case 复核后重新判定，当前主要阻塞仍是 RANS 收敛。
...
- bounded-CFL 上限10 的 500 iteration 末尾 `relrms[Rho]=-3.10767`，比上限5继续改善但仍未达到 `< -4`；
- bounded-CFL 上限20 的 500 iteration 末尾 `relrms[Rho]=-3.21966`，仅小幅改善；
- bounded-CFL 上限20 的 1000 iteration 末尾 `relrms[Rho]=-3.39242`，接近 max=50 的 -3.38664，但仍未达到 `< -4`；残差约在 -3.39 平台，CFL 已不再是首要瓶颈。

下一步读取 `TURBOMACHINERY` 输出和末尾阶段性能，检查物理量是否已形成稳定平台；未收敛前不称为 RANS 验证。
