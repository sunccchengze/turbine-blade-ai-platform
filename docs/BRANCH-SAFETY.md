...
## 4.5 过期文档的时效标记（2026-09-02 · 新会话必读）

仓库里有 20 份 8 月上旬的日期型文档（讲稿、答辩包、runbook、验收记录、30 天计划…），
里面仍有「SnapDeploy 手动 Redeploy」「先唤醒线上后端容器」「合入 PR 即开始预热」这类**当时成立、今天作废**的指令。
每份文件顶部已加 `⛔ 时效标记`，逐条写明「第几行 → 为什么作废 → 现行口径在哪」，**正文一字未改**（保留历程原貌）。

- 判据（可复核）：末次改动 < 2026-08-16 且 命中 `Redeploy|snapdeploy.app|/api/|VITE_API_URL|FastAPI|不推 main|合入 PR|先读本文件` 之一；
- 误伤已回滚：比喻性提到「冷启动」的大师笔记（`docs/GHG-*`）、当日正确的验收叙述、历史时间轴行，都不该标——见 `HANDOFF.md` §9 #19 的白名单；
- **读旧文档前先 grep 这两处**：`HANDOFF.md` §9.5（架构现状）、本文件（会话/git 纪律）。文档与它们冲突时，一律以这两处为准。
