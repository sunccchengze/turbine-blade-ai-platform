...
> ⛔ **时效标记（2026-09-02 追加 · 由分支收敛会话自动判定）** —— 本文件是 **2026-08-14** 的历史快照，**不是现状**。
> 以下写法在今天已经不成立：
> - 第 3 行「下一会话先读本文件，再读宪章和 skill。根目录 `HANDOFF.md` 仍是 2026-07/08 初的旧作战本，里面的「MDO 平台 / 本科二年级 / 100,000× 当贡献 / P1–P4 已 Gate」不」→ **本文件不是下一会话的入口：入口是 docs/BRANCH-SAFETY.md → docs/SESSION_HANDOFF-20260902.md**
> - 第 23 行「不推 `main`。不主动开/合 PR。」→ **git 纪律已由 docs/BRANCH-SAFETY.md 取代：快进推送可在不碰 PR 的前提下进 main**
>
