...
| 附面层 | boundary layer | 贴壁的薄粘性剪切层。壁面速度为 0（无滑移），向外恢复到主流。禁止说「负面层」 |
| 激波–附面层干扰 | SWBLI | 激波的逆压梯度打在附面层上，层变厚甚至分离 |
| 吸力面 / 叶背 | suction side | 叶片上压力较低、气流加速的那一面 |
| 压力面 / 叶盆 | pressure side | 叶片上压力较高的那一面 |
| 喉部 | throat | 相邻叶片通道最窄的截面。堵塞发生在这里 |
| 分离 | separation | 近壁流体不再贴着壁面向下游走，出现回流或脱离 |

仓库指认：示意图 `教材/figures/U04-shock-bl.svg`。代理最大流量点与 η 的配对在 `evidence/metrics.json` 的 `nsga2_surrogate`。
...
- 74 维统计把激波位置抹掉（U05）。所以网络用平滑函数贴一个接近跳的 \(\eta\)，残差堆在跳附近，Dropout 抖权重也抖不出那根红线——这是 U08–U09 的伏笔。
