...
| N-S 方程 | Navier–Stokes | 质量、动量、能量守恒加上本构。直接解所有湍流尺度叫 DNS，太贵 |
| RANS | Reynolds-Averaged Navier–Stokes | 对 N-S 做时间平均，脉动收进雷诺应力，再用湍流模型封闭 |
| 网格 | mesh | 把连续通道切成有限个单元，在单元上离散方程 |
| 残差 | residual | 离散方程左右两边还差多少。本章「收敛」只指残差降到阈值 |
| relrms | relative RMS | \(\log_{10}\|R^{(k)}\|/\|R^{(0)}\|\)。−3.39 表示残差只掉到约 \(4\times10^{-4}\) 倍 |
| null | null | 「这里空着、还没有填数」。不是 0，也不是考了 0 分 |
| 一阶迎风 | first-order upwind | 一种耗散大的离散格式，激波会被抹宽 |

仓库指认：`evidence/metrics.json` → `su2_coarse.relrms_platform = -3.39`，`converged: false`。`decision_metrics` 现在全部是 null。
