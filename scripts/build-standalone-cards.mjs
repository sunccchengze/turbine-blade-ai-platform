...
.progress-meta { display: flex; gap: 15px; color: #91aaa0; font: 10px var(--mono); white-space: nowrap; }
...
@media (max-width: 700px) {
  ...
  .progress-meta { display: grid; gap: 3px; justify-items: end; }
...
<main class="atlas-page">
  <div class="atlas-shell">
    ...
    <section class="progress-bar" aria-label="学习总进度">
      ...
      <div class="progress-meta"><span id="percent">0% complete</span><span id="reviewCount">↻ 0 待复习</span></div>
    </section>
    ...
    <div class="workspace">
      <section>
        <div class="stage-meta"><div class="phase-label"><i class="dot" id="phaseDot"></i><strong id="phaseTitle">公式桥</strong><small id="phaseEn">FORMULA BRIDGE</small></div><div class="shortcuts">← → 导航 · Space 显示答案 · R 返回正面</div></div>
...
function reviewCount() { return cards.filter(card => progress[card.id]?.status === 'review').length; }
...
function renderBack(card) {
  ...
  return '<div class="card-top"><span>' + esc(card.phaseLabel) + ' / CARD BACK</span><span class="card-id">' + statusText + '</span></div>' +
    ...
    '<div class="card-footer back-footer"><button class="secondary" id="frontButton" type="button">↶ 返回正面</button><div class="rating"><button class="review" id="reviewButton" type="button">↻ 需要复习</button><button class="master" id="masterButton" type="button">✓ 我能解释</button></div></div>';
}
function renderRoute() {
  ...
  list.querySelectorAll('.road-card').forEach(button => button.addEventListener('click', () => goTo(Number(button.dataset.index))));
}
function renderProgress() {
  const mastered = masteredCount();
  const reviewed = reviewCount();
  ...
  el('reviewCount').textContent = '↻ ' + reviewed + ' 待复习';
...
function renderCard() {
  ...
  el('cardNumber').textContent = 'Card ' + card.order + ' of ' + cards.length;
...
function mark(status) {
  const card = currentCard();
  progress[card.id] = { status: status, attempts: (progress[card.id]?.attempts || 0) + 1, updatedAt: new Date().toISOString() };
  saveProgress();
  stopTimer();
  renderCard();
}
...
el('previousButton').addEventListener('click', () => goTo(currentIndex - 1));
el('nextButton').addEventListener('click', () => goTo(currentIndex + 1));
...
window.addEventListener('keydown', event => {
  ...
  if (event.key === 'ArrowRight' && flipped) { event.preventDefault(); goTo(currentIndex + 1); }
...
await writeFile(new URL('../aero-atlas-cards.html', import.meta.url), output)
console.log('wrote aero-atlas-cards.html with', learningCards.length, 'cards')
