...
### 升级：统计特征 + 点云融合模型（Day 39）Surrogate v3: Fused

在**同样的留出测试集**上，由「统计特征（74 维）+ 点云几何（9 通道）」融合模型实测
（512 点 / 40 epoch / CPU，复现：`train_fused_p1.py`）：

| 输出 | 基线（74维MLP） | **融合模型 R²** | 提升 |
|---|---|---|---|
| 总压比 π | 0.9844 | **0.9718–0.9902** | 持平/超越 |
| 等熵效率 η | 0.9561 | **0.9608–0.9634** | **+0.005~0.007** |
| 质量流量 ṁ | 0.9827 | **0.9829–0.9869** | 持平/超越 |

> **课题级结论**：几何空间信息（点云）为性能预测提供统计特征之外的**增量价值**
> —— 融合模型稳定持平并局部超越纯统计特征基线（尤其 η）。
...
| 指标 | 最优值 | 相对训练集均值 |
|---|---|---|
| 等熵效率 η | **0.9173**（代理预测） | **+5.40%**（均值 0.8703） |
| 质量流量 ṁ | **21.74 kg/s**（代理预测） | **+11.43%**（均值 19.51） |
| 总压比 π | 2.1073 | 均值 1.9839 |

> ⚠️ **诚实披露（自查证据链，`pareto_evidence.py` 一键生成）**：
> 1. **范围合规**：100 个 Pareto 解的全部 74 维特征均落在全量 1,000 组观测范围内（0 越界）——
>    可实现性底线成立；
> 2. **流形位置**：Pareto 解到训练集最近邻的中位距离 ≈ 17.4（标准化空间），约为训练集内部
>    最近邻中位距离（6.1）的 3× —— **前沿位于数据流形边缘**；
> 3. **外推量**：η 预测超出最近邻训练样本实测中位 0.0235（≈ 测试集 MAE 的 14×）——
>    「超越训练样本」的收益主要来自流形边缘的外推，**不是训练数据内的插值**；
> 4. **邻域误差**：按「留出测试样本到 Pareto 前沿距离」分层，四层真实 MAE 与全测试集持平
>    （η 0.0013–0.0023，Spearman 相关 ≈ 0），即代理模型在 Pareto 邻域**没有系统性恶化**。
>
> **结论**：以上数字是代理模型的预测，误差水平经留出数据验证未恶化，但最终裁决必须由
> **RANS 抽查**（P4，SU2 环境已就绪）给出。答辩与网站均采用「代理模型预测」措辞。
...
> ⚠️ **复现必须先装完整锁版环境**：请用 `backend/requirements.txt` 全量安装
> （`pymoo==0.6.1` / `numpy==1.26.4` / `scipy==1.12.0` / `onnxruntime==1.18.0` /
> `scikit-learn==1.7.2`）。**任何版本偏离都会改变 NSGA-II 结果**——实测 pymoo 0.6.2
> 使 max η 漂到 0.9212、numpy 2.x 漂到 0.9191、onnxruntime 1.28 漂到 0.9180；
> 仅在全锁版下复现 0.9173 且与仓库 CSV 逐位一致。CI 作业 `verify.yml` 持续验证两套数字。
...
2. **代理模型不替代 CFD** —— 它的定位是**设计前端的快速筛选器**：
   ...
   NSGA-II 找到的 Pareto 前沿位于训练数据流形边缘（自查见优化节），
   「超越训练样本」的结论以代理预测为口径，RANS 终审进行中（P4）。
...
> **口径说明**：Day N 为**工作单元计数**（每个 Day = 一个完整开发/验收单元，对应 git 历史中的一组提交），
> 实际全部完成于 **2026-07-30 至 2026-08-01 三个日历日**（AI 辅助高强度开发）。
> 完整逐日记录见 `docs/devlog/`，每个条目对应可点击的真实提交 SHA。
