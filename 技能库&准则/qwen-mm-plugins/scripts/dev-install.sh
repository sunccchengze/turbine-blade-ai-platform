#!/usr/bin/env bash
# Editable-install qwen-mm-plugins into the active environment, so you can `import qwen_mm_plugins_core`
...
echo "✓ editable-installed qwen-mm-plugins${extras:+[$extras]}"
echo "  try: python -c 'import qwen_mm_plugins_core as p; print(len(p.SPECS), \"tools\")'"
