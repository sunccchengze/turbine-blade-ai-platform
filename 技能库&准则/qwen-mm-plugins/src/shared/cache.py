...
def cache_dir() -> str:
    """Directory for derived render artifacts, overridable via QWEN_MM_CACHE.

    - macOS:   ~/Library/Caches/qwen-mm-plugins
    - Linux:   $XDG_CACHE_HOME/qwen-mm-plugins  (or ~/.cache/qwen-mm-plugins)
    - Windows: %LOCALAPPDATA%\\qwen-mm-plugins\\cache
    """
    override = get_env("QWEN_MM_CACHE")
    if override:
        base = override
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA") or os.path.expanduser(r"~\AppData\Local")
        base = os.path.join(local, "qwen-mm-plugins", "cache")
    elif sys.platform == "darwin":
        base = os.path.expanduser("~/Library/Caches/qwen-mm-plugins")
    else:
        xdg = os.environ.get("XDG_CACHE_HOME") or os.path.expanduser("~/.cache")
        base = os.path.join(xdg, "qwen-mm-plugins")
