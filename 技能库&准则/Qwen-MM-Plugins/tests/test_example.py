...
conftest auto-discovers qwen_mm_plugins_example (it scans src/capabilities/*/ for the server
package), so it imports like any other server.
"""

import json

import qwen_mm_plugins_example as ex
...
def test_config_probe_reports_env_with_default(monkeypatch):
    monkeypatch.delenv("QWEN_MM_EXAMPLE_GREETING", raising=False)
