...
description: Example/template capability — a minimal MCP server whose demo tools (echo, make_swatch, make_film_strip, describe, config_probe) show how a Qwen-MM-Plugins capability is structured: the content-return shapes, calling an API, reading env/config, and composing other capabilities. Use it as the starting point when adding a new capability.
---
...
You have the `qwen-mm-plugins-example` MCP tools available:
...
**environment variable > `~/.qwen-mm-plugins/config` > default**. For example, `describe` gets
`DASHSCOPE_API_KEY` this way (through `resolve_openai_endpoint`), and `config_probe` reads the demo
tunable `QWEN_MM_EXAMPLE_GREETING`. Run **config_probe** to see what currently resolves.
...
- If the user gives a **video**, first call **`read_video`** from **`qwen-mm-plugins-core`** to
  extract frames, then feed a frame path to `describe` (to caption it) or `make_swatch`.
- If the user gives a **document / PDF / 3D model**, use `qwen-mm-plugins-core`'s reading tools the
