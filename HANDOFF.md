...
0. **强制讲课 skill（2026-08-13 A1，高于旧「小白类比」）**  
   - 读 `docs/agent-charter/chengze-deep-tutor.md`  
...
**旧分支处置（2026-09-02）**：10 条 `arena/019f*` 已删除，远端只剩 `main` 与会话分支（同指一点）；`archive/*` tag 10 个全部保留作复原入口。
线上侧：main 前进后的 Pages 自动构建**已人工验收为绿**；`技能库&准则/` 的 1.29 GB 经承泽决定**不做瘦身**。
